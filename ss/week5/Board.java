...
public class Board {
    ...
    public boolean hasRow(Mark m) {
        for (int i = 0; i < (DIM*DIM); i += DIM) {
        	if (getField(i) == m && getField(i) == getField(i+1) && getField(i) == getField(i+2)){
        		return true;
        	}
        }
    	return false;
    ...
    public boolean hasColumn(Mark m) {
    	for (int i = 0; i < DIM; i++) {
    		if (getField(i) == m && getField(i) == getField(i+DIM) && getField(i) == getField(i+2*DIM)){
    			return true;
    		}
    	}
        return false;
    ...
    public boolean hasDiagonal(Mark m) {
    	...
    	for (int i = 1; i < DIM; i++){
    		if (getField(i,i) == m && getField(i - 1,i - 1) == getField(i, i)){
    			downCounter++;
    		}
    		if (getField(i - 1, DIM - i) == m && getField(i - 1, DIM - i) == getField(i, DIM - (1 + i))) {
    			upCounter++;
    		}
    	}
    	return (upCounter >= DIM || downCounter >= DIM);
    }
